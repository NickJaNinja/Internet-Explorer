package com.example.spacetraders.views;

<<<<<<< HEAD
import android.support.v7.widget.RecyclerView;
import android.widget.TextView;

import com.example.spacetraders.entities.Planet;
import com.example.spacetraders.models.Model;

import java.util.List;

/**
 * ship adapter class
 */
public class ShipAdapter {
/*
public class ShipAdapter extends RecyclerView.Adapter<ShipAdapter.ShipViewHolder> {

    private enum shipType{};
    private ShipAdapter.OnClickListener listener;
    private Model model;
    private Ship selectedPlanet;


    public class ShipViewHolder extends RecyclerView.ViewHolder {
        private TextView name;
        private TextView numWeapons;
        private TextView numShields;
        private TextView numGadgets;
        private TextView numCargoHolds;
        private TextView fuel;
        private TextView numCrew;

    }
}
*/
}
